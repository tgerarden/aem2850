...
{{< pagebreak >}}
## 2. Modify your code from 1. to make sure that tickers that are absent from the data are returned rather than removed. Test it out with the tickers `c("AAPL", "DYSON", "TSLA")`.

*Hint: at the end of the function, you could use `tibble()` to create a data frame with the column `symbol` that contains the contents of `ticker`, and then join this data frame and the data frame of annual returns.*
