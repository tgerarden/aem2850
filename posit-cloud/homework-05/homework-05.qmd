...
```{r echo = TRUE}
aapl_news <- _____("aapl_news.csv")

_____ |>
  _____(sentiment, sort = TRUE) |>
  mutate(share = _____ / _____(_____) * 100)
```
...
```{r echo = TRUE}
aapl_news |>
  mutate(other_tech = _____(
    headline,
    regex(_____, _____ = _____)
  )) |>
  group_by(other_tech) |>
  summarize(share_positive = mean(_____==_____) * 100)
```
...
iphone_releases <- iphone_releases |>
  mutate(
    release_month = str_extract(_____, _____)
    release_year = as.integer(_____(_____, _____)),
  ) |> 
