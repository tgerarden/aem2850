...
library(scales) # plot functions like breaks_width, label_date, label_currency
...
## Call the function with the data frame `prices_1y` and ticker "XOM" to compute annualized returns for Exxon Mobil over the last year of the data.
```{r}
...
## Call the function with the data frame `prices_ytd` and ticker "XOM" to compute annualized returns for Exxon Mobil over the calendar year so far (year to date).
```{r}
...
Here is some code that imports and tidies Zillow sales price data in a few steps:

1. importing a file based on the string it starts with,
2. dropping the observations for the region of "United States",
3. separating `RegionName` into two variables `City` and `State`,
4. pivoting the data longer, and
5. converting `date` into date format and generating the variables `year` and `month`.

```{r message = FALSE}
raw_tidy_example <- "Metro_median_sale_price_uc_sfrcondo_month.csv" |> 
  read_csv() |> 
  filter(RegionName != "United States") |> 
  separate_wider_delim(RegionName, delim =", ", names = c("City","State")) |>
...
## 6. Modify the function in question 5 so that it can be used to tidy either price data or sales data, based on a second argument that specifies which one the user is asking for. What years do the sales data cover?
...
<!-- Note: The code chunk options below are just used to simplify the output. -->
...
``` {r, warning=FALSE, message=FALSE, fig.show="hold", out.width="45%"}
# map(
