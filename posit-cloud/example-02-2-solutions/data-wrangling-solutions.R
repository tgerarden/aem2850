...
college_salaries |>
  filter(major_category == "Business") |> # subset majors within Business
  select(major, median) # select major and median
...
# 1.4 dplyr::summarize -----
# use dplyr::summarize to add up the number of people across all majors
college_majors |>
...
college_majors |>
  ...
  summarize(total = sum(total, na.rm = TRUE)/10^3)

# do it again, but now sort the data and compute category shares
college_majors |>
  group_by(major_category) |>
  summarize(total = sum(total, na.rm = TRUE)/10^3) |>
  arrange(desc(total)) |> # sort categories from most to least students
  mutate(share = total / sum(total) * 100) # compute shares in percentage points
...
# Engineering has the largest number of individual majors

# is that the same major category that has the largest number of students?
# No, Business has the largest number of students
...
map("state", region = "new york") # insert your code to make a map of new york here
...
map("world", region = "thailand") # insert your code to make a map of a country here
# if it doesn't render properly the first time, try entering the command again
