...
Remaining deadlines:
  ...
  - Prelim 2 on May 7 in class (two weeks from today)
...
tidy_big_red <- big_red_table |> 
  pivot_longer(everything(), names_to = "pronoun", values_to = "sport") |> 
...
What function(s) could we use to determine how many pronoun-sport pairs there are in `tidy_big_red`?
...
.small[
*The site has changed, so we will scrape from a 2022 archive of it using the [The Wayback Machine](https://web.archive.org/web/20220405170508/https://en.wikipedia.org/wiki/College_and_university_rankings_in_the_United_States)*
]
...
# below we use `[]` syntax to extract the tables by index
