...
<!-- No homework due next Monday because... -->

Prelim 1 will be next Thursday, October 2 at 7:30pm in Warren 173 and 175
  - Will cover everything so far **including this week's material**
  - Tuesday, Sep 30: review session in class
...
```{r, eval=FALSE}
theoffice |> slice_head(n = 1) |> 
    mutate(line = str_c(character, " said: ", text)) #<<
```
...
```{r, eval=FALSE}
theoffice |> slice_head(n = 3) |> 
  mutate(line = str_glue("{character} said: {text}")) # note the different syntax #<<
```
...
Note that quantifiers modify the pattern they follow: we used `.*`, not `*.`
...
theoffice |> select(season, episode, character, text) |> 
  filter(str_detect(text,    # where to match a pattern: the column named "text" #<<
                    "sale")) # what pattern to match: the pattern "sale" #<<
```
