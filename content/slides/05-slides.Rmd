...
```{r}
characters <- theoffice |> distinct(character) |> slice_head(n = 10) |> pull(character)
characters
```
...
.pull-left[
```{r, eval=FALSE}
str_view(characters, pattern = "J.m")
```
]

.pull-right[
```{r, eval=FALSE}
str_view(characters, pattern = "J.n")
```
]
...
```{r, eval=FALSE}
str_view(characters, "M.*l") # match strings with M, then any number of any characters, then l
```
...
```{r}
str_view(characters, "[aeiou]") # vowels
```
]
...
```{r}
str_view(characters, "[^aeiou]") # NOT vowels
```
]
...
`|` allows you to search for one or more alternative patterns
...
.pull-left[
```{r, eval=FALSE}
str_view(characters, "J.m|P.m")
```
]

.pull-right[
```{r, eval=FALSE}
str_view(characters, "S.*|O.*")
```
]
...
See [Chapter 17 of R4DS (2e)](https://r4ds.hadley.nz/regexps.html#pattern-details) for more on:
- **escaping**: matching meta-character as if they were literal strings
...
Reminder: `[]` let you match a set of characters; `^` inverts character class matches
...
```{r}
characters
str_replace(characters, "Dw", "Duhw") # jim's office pronunciation guide
```
