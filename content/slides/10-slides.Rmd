...
.pull-right[
```{r print-cars-model, highlight.output=c(2,3,5,6,7)}
car_model
```
]
...
```{r tidy-car-model-geom_smooth-code, eval=FALSE, fig.dim=c(4.8, 4.2), out.width="100%"}
mpg |> 
...
.pull-right[
```{r tidy-car-model-geom_smooth-plot, echo=FALSE, fig.dim=c(4.8, 4.2), out.width="100%"}
mpg |> 
  ggplot(aes(x = displ, y = hwy)) +
  geom_point() +
  geom_smooth(
    method = "lm",     # smoothing function #<<
    formula = "y ~ x", # optional (default) #<<
    fullrange = TRUE,  # start line at 0 #<<
...
.left-code[
```{r mfx-plot-cars-interactions, tidy=FALSE, message=FALSE, fig.show="hide", fig.dim=c(4.8, 3), out.width="100%"}
...
.right-plot[
![](`r knitr::fig_chunk("mfx-plot-cars-interactions", "png")`)
