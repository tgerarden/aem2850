...
See [aem2850.toddgerarden.com/schedule](https://aem2850.toddgerarden.com/schedule/) for minor schedule updates
...
# could switch to use youtube html embed code next year to just do this right in the slides
```
...
.left-code[
.box-2.small[Map wealth to the x-axis, health to the y-axis, add points, color by continent, size by population, scale the x-axis with a log, and facet by year]
...
.small[This terrible figure brought to you by a former AEM 2850 student!]
]
...
gapminder_ordered |> 
  ggplot(aes(x = continent, 
             y = n,
             fill = continent)) +
  geom_waffle(aes(values = n), # waffle aes() #<<
              n_rows = 10,     # waffle options #<<
              flip = TRUE) +   # make vertical #<<
  labs(fill = NULL) +
  coord_equal() + # make the boxes square
...
# example-07:<br>amounts-practice.R
