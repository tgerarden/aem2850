...
hp1_data |> 
  ...
  anti_join(stop_words, join_by(word1 == word)) |> # filter out #<< 
  anti_join(stop_words, join_by(word2 == word)) |> # stop words #<< 
  unite(bigram, word1, word2, sep = " ") |> # recombine #<<
  count(bigram, sort = TRUE) # count
```
]
...
Move beyond simple frequency plots to analyze relative frequencies by groups:
...
.less-right[
```{r hp-he-she-countsr, cache=TRUE, echo=FALSE}
hp_pronouns <- c("he", "she") # not apologizing for j k rowling here
...
2: Compute group-specific frequencies for the most common bigrams
...
3: Compute log ratios for the most common bigrams
...
4: Rearrange the data for plotting
...
.pull-right[
```{r hp-he-she-plotr, cache=TRUE, echo=FALSE, fig.width=8, fig.height=3.75, out.width="85%"}
plot_word_ratios |> 
  ggplot(aes(logratio, 
             fct_reorder(word2, logratio), 
             fill = logratio < 0)) +
