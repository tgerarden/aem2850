...
{{< pagebreak >}}
## 1. Import `Internet access in Brazil (2022-2023).xlsx` and assign it to a new object named `access`. You may need to specify the range of cells in the spreadsheet that should be imported. We'll consider the level of observation to be a state-year-month.
...
access2 <- access |>
  ...
  mutate(
    year = _____(_____) %/% 100,
    month = _____(_____) %% 100
  )
...
{{< pagebreak >}}
## 3. We'll ignore the distinction between area types (rural, suburban and urban), so aggregate the data up to the `state`-`provider`-`remote_area`-`year`-`month` level by grouping and summarizing appropriately (watch out for missing data!). Ungroup the data frame after summarizing. You do not need to retain `year_month`. Assign the resulting data frame to `access3` and print it.
...
{{< pagebreak >}}
## 4. Finish tidying this data set by using `pivot_wider` so that `access3` shows for any given state, year, and month: (i) the number of Starlink connections in remote areas; (ii) Starlink connections in non-remote areas; (iii) connections to other providers in remote areas; and (iv) connections to other providers in non-remote areas. Assign the resulting data frame to `access3` and print it.
...
## Import `brazil_regions.csv` and assign it to a new object named `regions`. The Brazilian Amazon is mostly contained in the North region, so we want to single out states that are located there. With that in mind, is this data frame tidy? If not, why is that a problem, given what we want to do with this data?
...
{{< pagebreak >}}
## 7. Let's go back to the internet data. Filter `access3` so that only North states remain in the data frame. When was Starlink first introduced in each state? You can present the answer to this question in a small data frame with 7 rows.
<!-- Tip: As we saw in a previous example, you can use the syntax `%in%` in conjunction with `filter` to identify the rows where `state` takes on a value contained in `north_states`. -->
