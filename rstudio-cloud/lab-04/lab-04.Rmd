...
<!-- Tip: You can use class() to find the data type. Use as.numeric() to convert to the numeric type. -->
...
<!-- Tip: This question is similar to questions #7 and #8 of Lab 3. Please refer to Lab-03-solutions if you have no clue for this one.-->
...
<!-- Tip: From question #6, we have found the three most populous cities in Taxes. We can use semi_join() to filter the `tidy_txhouse` data to get the observations only for these five cities. Then combine group_by() and summarize() to get the total sales of each city over 2010-2015. -->
...
top3_sales <- tidy_txhouse %>% 
  ...
  arrange(desc())
```
The total sales are `r top3_sales$sales[1]` in `r top3_sales$city[1]`, `inline code` in `inline code`, and `inline code` in `inline code`.
...
## 8. Create a data frame of sales, price, and population for all the city-year pairs that appear in both `tidy_txpop` and `tidy_txhouse`. Of these, which city had the most sales per capita in 2010? Can you think of a reason that place might have the most sales per capita?
<!-- No tips this time! -->

```{r echo = TRUE}

```
The place with the most sales per capita in 2010 was `r inline code here`.

One possible reason this place had the most sales per capita is...
