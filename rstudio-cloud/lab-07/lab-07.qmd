...
## 1. In `diamonds`, one variable `color` takes on the values `r unique(diamonds$color)`. Count the number of diamonds of each color. Use `goem_col` to plot a bar chart, with x-axis being the level of `color` of a diamond, and y-axis the number of diamonds per `color`. Fill the bars by `color`. Label the x-axis "Color", and the y-axis "Number of diamonds".
...
{{< pagebreak >}}
## 3. Calculate the average price of by diamonds color. Use `geom_pointrange` to plot a lollipop chart, with the x-axis being the color of the diamonds, and the y-axis being the average price. Add labels to both axes.
<!-- No tips this time! -->
...
{{< pagebreak >}}
## Questions 7-8 are not specific to amounts, but they provide more practice with data wrangling and dataviz. The **nycflights13** provides a data set `weather` that includes weather condition variables (temperature, humidity, wind direction, wind speed, precipitation, pressure, visibility, etc.) by origin-year-month-day-hour.
