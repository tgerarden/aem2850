...
# now combine dplyr::filter with dplyr::select
# to print just the major and median columns for majors within Business
college_salaries %>% 
  ______(major_category ______ "______") %>% # filter to get majors within Business
...
business_majors <- college_salaries %>% 
  ______(major_category ______ "______") %>% # filter to get majors within Business
  select(______) # select major

# let's zoom back out
# what are the top 10 majors by median salary? no hints this time!
college_majors %>% 
  filter(______ ______ ______) %>% 
  select(major)
...
college_majors %>% 
  ...
  ______(desc(______)) %>% # arrange categories from most to least students
